...
const createInspectionHTML = (data) => {
  const logoBase64 = '/logo.png'; // Use relative path to public folder
...
const createQualityHTML = (data) => {
  ...
  return `
    <html>
      <head>
        <style>
          body { font-family: 'Helvetica', 'Arial', sans-serif; color: #333; margin: 0; padding: 40px; -webkit-print-color-adjust: exact; }
      ...
      <body>
        <div class="header">
          <div>
            <img src="${logoBase64}" style="width: 80px; height: auto; margin-bottom: 10px;" />
...
export const downloadPDF = (type, data) => {
  console.log('Starting PDF download...', type);
  console.log('html2pdf type:', typeof html2pdf);

  if (typeof html2pdf === 'undefined') {
    alert('Error: html2pdf library is not loaded properly.');
    return;
  }
  ...
  try {
    ...
    if (type === 'inspection') {
      ...
      fileName = `inspection_${data.truck_number || 'TRUCK'}_${new Date(data.created_at).toISOString().split('T')[0]}.pdf`;
    } else if (type === 'quality') {
      htmlContent = createQualityHTML(data);
      fileName = `quality_${data.truck_number || 'TRUCK'}_${new Date(data.created_at).toISOString().split('T')[0]}.pdf`;
    } else {
    ...
    const pdfLib = html2pdf.default || html2pdf;
    if (typeof pdfLib !== 'function') {
      alert('html2pdf is not a function. Check console for details.');
      console.error('html2pdf export:', html2pdf);
      return;
    }

    const worker = pdfLib().set(opt).from(htmlContent).save();

    // worker is a promise-like object
    if (worker && typeof worker.then === 'function') {
      worker.then(() => {
        console.log('PDF saved successfully.');
      }).catch((err) => {
        console.error('html2pdf worker error:', err);
